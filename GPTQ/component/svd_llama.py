...
class SVD_LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None, ratios=[1,1], layers_cond=None):
        super().__init__()
        self.config = config
        ...
        self.q_u_proj = nn.Linear(low_rank_qk, self.num_heads * self.head_dim, bias=False)
        self.q_v_proj = nn.Linear(self.hidden_size, low_rank_qk, bias=False)

        self.k_u_proj = nn.Linear(low_rank_qk, self.num_heads * self.head_dim, bias=False)
        self.k_v_proj = nn.Linear(self.hidden_size, low_rank_qk, bias=False)
        if 'ov_proj' in self.layers_cond:
            self.v_u_proj = nn.Linear(low_rank_ov, self.num_heads * self.head_dim, bias=False)
            self.v_v_proj = nn.Linear(self.hidden_size, low_rank_ov, bias=False)
            
            self.o_u_proj = nn.Linear(low_rank_ov, self.hidden_size, bias=False)
            self.o_v_proj = nn.Linear(self.num_heads * self.head_dim, low_rank_ov, bias=False)
        else:
