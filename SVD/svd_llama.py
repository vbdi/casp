...
def whitening(model_name, model, profiling_mat, ratio, dev,layers_cond):
    ...
    for i in tqdm(range(len(layers))): 
        ...
        svd_attn = SVD_LlamaAttention(config=model.config, ratio=ratio,layers_cond=layers_cond)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ratio',type=float, help='Target compression ratio of Wq and Wk')
    ...
    if args.save_path is not None:
        torch.save({'model': model, 'tokenizer': tokenizer}, args.save_path + "/" + args.model.replace("/", "_").replace("-", "_") + str(args.ratio) + 'QK.pt')   # fp32