...
def get_inps(
    ...
    layers = get_layers(model.language_model)
    ...
    if emb_device.type != "cuda":
        ...
        if model.language_model.config.model_type == "opt":
    ...
    inps = [
        torch.zeros(
            (min(nsamples_per_device, len(data) - i * nsamples_per_device), model_seqlen, model.language_model.config.hidden_size),
            dtype=dtype,
    ...
    if model.language_model.config.model_type.lower() in FALCON_TYPES:
...
def quantize_aq(model: PreTrainedModel, data: Sequence, val_data: Optional[Sequence], args: Namespace):
    ...
    for layer_index in range(len(layers)):
        ...
        if args.true_sequential:
            sequential = get_sequential_groups(model.language_model,args.svd)
        else:
        ...
        for names in sequential:
            ...
            if len(args.devices) == 1:
                ...
                aq_handlers = init_aq_engines(
                    ...
                    [
                        name
                        for name in names
                        if ((".gate" not in name.lower()) or ("mixtral" not in model.language_model.config.model_type.lower()))
                    ],
                    inps[0],
                    outs[0],
                    **forward_args,
            ...
            for sublayer_name in aq_handlers.keys():
                if args.svd:
                    if args.tbits==2:
                        if '13b' in args.model_path: 
                            if layer_index<9 or layer_index==39 or "_v_proj" in sublayer_name.lower() or "_u_proj" in sublayer_name.lower():
                            ...
                            # rand=[13,26,27,31,8,11,16,6]
                            # if layer_index in rand or "_v_proj" in sublayer_name.lower() or "_u_proj" in sublayer_name.lower():
                            if layer_index<7 or layer_index==31 or "_v_proj" in sublayer_name.lower() or "_u_proj" in sublayer_name.lower():
                                args.num_codebooks=2 
                                args.nbits_per_codebook=12 
                                args.in_group_size=8
                                num_codebooks = args.num_codebooks
                        ...
                        if args.tbits==3:
                            if '7b' in args.model_path: 
                                if layer_index<5 or layer_index==31 or "_v_proj" in sublayer_name.lower() or "_u_proj" in sublayer_name.lower():
                                    args.num_codebooks=2 
                ...
                if "mixtral" in model.language_model.config.model_type.lower() and args.mix_compression:
                    assert "mixtral" in model.language_model.config.model_type.lower()
        ...
        if len(args.devices) == 1:
            ...
            out_losses = update_outs(layer, inps[0].float(), outs[0].float(), compute_mse=should_compute_mse, **forward_args)
        else:
        ...
        if run_validation:
            if len(args.devices) == 1:
                ...
                out_val_losses = update_outs(
                    layer, val_inps[0].float(), val_outs[0].float(), compute_mse=should_compute_mse, **forward_args
                )
            else:
...
def perplexity_eval(model: PreTrainedModel, testenc: torch.LongTensor, args: Namespace) -> float:
    ...
    inps, forward_args = get_inps(model.cuda(), testenc, args.model_seqlen, args.devices, args.offload_activations)
    outs = [torch.zeros_like(inp_tensor, pin_memory=inp_tensor.is_pinned()) for inp_tensor in inps]
    ...
    for i in trange(len(layers), desc="processing eval data by layer"):
        ...
        if len(args.devices) == 1:
            assert len(inps) == len(outs) == 1
            update_outs(layer, inps[0].float(), outs[0].float(), compute_mse=False, **forward_args)
        else:
    ...
    get_model_head_with_norm(model.language_model).to(device)
    ...
    for i in range(nsamples):
        ...
        lm_logits = get_lm_logits(inp.to(device), model.language_model)
        ...
        print('######### PPL:',torch.exp(torch.stack(nlls).sum() / (len(nlls) * args.model_seqlen)).item())
...
def update_outs(
    ...
    for j in trange(len(inps_tensor), desc="calc outs after quantization", leave=False):
        outs_batch = layer(inps_tensor[j].to(device).unsqueeze(0).half(), **forward_args)[0]
        if compute_mse:
...
def update_outs_parallel(
    ...
    **forward_args,
    ...
    kwargs_by_device = []
    for i in range(len(devices)):
        inputs_by_device.append((layer_replicas[i], inps[i].half(), outs[i].half(), compute_mse))
        kwargs_by_device.append(
            {
                k: (v.to(devices[i], non_blocking=True) if isinstance(v, torch.Tensor) else v)
                for k, v in forward_args.items()
...
def main():
    ...
    parser.add_argument(
        "dataset",
        type=str,
        help="Dataset name [c4, pajama,LLaVAVideo178K] or path to data where to extract calibration data from.",
    )
    parser.add_argument(
        "--new_eval",
        action="store_true",
    ...
    parser.add_argument("--save_fake", type=str, default=None, help="Path to save quantized statistics.")
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument(
        "--svd",
        action="store_true",
        help="Whether the attention is compressed with svd",
    )
