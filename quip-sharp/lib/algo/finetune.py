...
def quantize_finetune_decoder_layer(mixed_layer, quant_order, idx, cbs, args,
    ...
    for quant_i, (linear_attr, name) in enumerate(quant_order):
        ...
        if name in ['q_u','q_v','k_u','k_v','qkv']:
            shared_args = (cbs[1].codesz, cbs[1].packsz, cbs[1].pack_out, str(cbs[1].idx_dtype),
                        cbs[1].version)
            cb=cbs[1]
            shared_kwargs['resid_scale_override']=args.resid_scale_override_qk
        elif name in ['o','v']:
            shared_args = (cbs[2].codesz, cbs[2].packsz, cbs[2].pack_out, str(cbs[2].idx_dtype),
                        cbs[2].version)
            cb=cbs[2]
            shared_kwargs['resid_scale_override']=args.resid_scale_override_ov
        else:
            shared_args = (cbs[3].codesz, cbs[3].packsz, cbs[3].pack_out, str(cbs[3].idx_dtype),
                        cbs[3].version)
            cb=cbs[3]
            shared_kwargs['resid_scale_override']=args.resid_scale_override_mlp
        ...
        setattr(
            ...
            quant_linear)
        if quant_i < len(quant_order) - 1 and args.ft_epochs>0:
