...
class LlamaMLP(nn.Module):
    ...
    def __init__(self, config,layer_idx):
        ...
        self.upgate_proj = FusedQuantizedLinear(
            ...
            config.quip_params[f'codesz_mlp_{layer_idx}'],
            config.quip_params.get(f'packsz_mlp_{layer_idx}', 1),
            config.quip_params.get('pack_out', False),
            config.quip_params[f'idx_dtype_mlp_{layer_idx}'],
            config.quip_params.get(f'codebook_version_mlp_{layer_idx}', 0),
            rank=config.quip_params['lora_rank'],
            rescale_WH=config.quip_params['rescale_WH'],
            resid_scale_override=config.quip_params.get(
                f'resid_scale_override_mlp_{layer_idx}', -1),
...
class SVD_LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None, ratios=[0.25,1], layers_cond=None):
        super().__init__()
        self.config = config
        ...
        self.ratios = ratios # 1 means no truncate, just keep normal attn
        ...
        low_rank_qk = int(self.hidden_size * self.ratios[0]/2)
        low_rank_ov = int(self.hidden_size * self.ratios[1]/2)

        self.q_u_proj = QuantizedLinear(
            low_rank_qk, 
            self.num_heads * self.head_dim,
            config.quip_params[f'codesz_qk_{layer_idx}'],
            config.quip_params.get(f'packsz_qk_{layer_idx}', 1),
            config.quip_params.get('pack_out', False),
            config.quip_params[f'idx_dtype_qk_{layer_idx}'],
            config.quip_params.get(f'codebook_version_qk_{layer_idx}', 0),
            rank=config.quip_params['lora_rank'],
            rescale_WH=config.quip_params['rescale_WH'],
            resid_scale_override=config.quip_params.get(
        ...
        self.v_proj = QuantizedLinear(
            ...
            config.quip_params[f'codesz_ov_{layer_idx}'],
            config.quip_params.get(f'packsz_ov_{layer_idx}', 1),
            config.quip_params.get('pack_out', False),
            config.quip_params[f'idx_dtype_ov_{layer_idx}'],
            config.quip_params.get(f'codebook_version_ov_{layer_idx}', 0),
            rank=config.quip_params['lora_rank'],
            rescale_WH=config.quip_params['rescale_WH'],
            resid_scale_override=config.quip_params.get(
