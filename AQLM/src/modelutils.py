...
from transformers import AutoConfig, AutoModelForCausalLM, LlavaNextVideoForConditionalGeneration
...
def get_model(
    ...
    with suspend_nn_inits():
        try:
            if  "llava-next-video" in model_path.lower(): 
            ...
            elif  "llava-1.5" in model_path.lower() or "llava_1.5" in model_path.lower():
                from transformers import AutoProcessor, LlavaForConditionalGeneration
                model = LlavaForConditionalGeneration.from_pretrained(model_path, device_map="cpu",torch_dtype=torch.float16,attn_implementation="eager")
            ...
            model = torch.load(model_path)['model'].half()
        if load_quantized:
...
def get_sequential_groups(model,svd=False):
...
def load_linear_layers(layer, quant_layer, model):
    # if 'llava' in model.config.model_type:
...
def load_quantized_model(model, load_path):
    ...
    if 'llava' in model.config.model_type:
        ...
        model_lm=model
    for layer_index in range(len(model_lm.model.layers)):
        model_lm.model.layers[layer_index] = torch.load(
            os.path.join(load_path, str(layer_index) + ".pth"),
            map_location=model_lm.model.layers[layer_index].input_layernorm.weight.device,
        )
        for module in model_lm.model.layers[layer_index].modules():
            if isinstance(module, QuantizedWeight):
                if not hasattr(module, "codes_storage"):
                    module.codes_storage = None  # backwards compatibility

    if 'llava' in model.config.model_type:
        model.language_model=model_lm
    else:
        model=model_lm
...
def save_not_quantized_weights(model: nn.Module, save_dir: str):
    ...
    if 'llava' in model.config.model_type:
        ...
        model_lm=model
    for layer in get_layers(model_lm):
