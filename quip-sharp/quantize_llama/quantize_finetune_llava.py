...
def quantize_llama_layer(layer, idx, cbs, args, device, pre_orig_emb, orig_emb,
    ...
    with torch.no_grad():
        if args.compressed_model:
            ...
            cur = 0
            for w in weights:
                print('##########',fused_qkv_proj.weight.shape,w.shape)
...
def main(args):
    ...
    args.codebook_qk=[args.codebook_qk]*len(model.language_model.model.layers)
    args.codebook_ov=[args.codebook_ov]*len(model.language_model.model.layers)
    args.codebook_mlp=[args.codebook_mlp]*len(model.language_model.model.layers)
    args.resid_scale_override_qk=[args.resid_scale_override_qk]*len(model.language_model.model.layers)
    args.resid_scale_override_ov=[args.resid_scale_override_ov]*len(model.language_model.model.layers)
    args.resid_scale_override_mlp=[args.resid_scale_override_mlp]*len(model.language_model.model.layers)
    ...
    orig_emb_cache = [model.language_model.model.embed_tokens(devset)]
    ...
    for i in range(len(model.language_model.model.layers)):
        ...
        all_config['model_config'].text_config.update({'quip_params': quip_params})
        ...
        if args.ft_epochs > 0:
            ...
            model.language_model.model.layers[i].to(cur_device)
            ...
            model.language_model.model.layers[i].cpu()
        ...
        proc_list[cur_device] = mp.Process(target=quantize_llama_layer,
                                           args=(
                                               ...
                                               orig_emb_cache[cur_device],
                                               orig_emb_cache[cur_device + 1],
                                               all_config['model_config'].text_config,
                                           ))
