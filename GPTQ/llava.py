...
def get_llava(model,args,seqlen):
    import torch
    ...
    if args.svd:
        model = torch.load(model)['model'].half()

    elif  "llava-next-video" in args.model.lower():
        from transformers import LlavaNextVideoForConditionalGeneration
        model = LlavaNextVideoForConditionalGeneration.from_pretrained(model, torch_dtype=torch.float16, use_flash_attention_2=False).to('cuda')

    elif  "llava-1.5" in args.model.lower():
        from transformers import  LlavaForConditionalGeneration
        model = LlavaForConditionalGeneration.from_pretrained(model, device_map="cpu",torch_dtype=torch.float16,attn_implementation="eager").to('cuda')
...
def llama_sequential(model, dataloader, dev):
    ...
    # dataloader[0]['input_ids']
    inps = torch.zeros((args.nsamples, model.seqlen, model.language_model.config.hidden_size), dtype=dtype, device=dev)
    ...
    for batch in dataloader:
        try:
            model=model.to(dev)
            model(**batch.to(dev)) #**batch.to(dev)
        except ValueError:
            pass
    ...
    for i in range(len(layers)): #:
        if args.svd:
            if args.tbits==2:
                ...
                if '7b' in args.model.lower():
                    ex=[4,16,2,21,1,15,28,22,17,24,21]
                    # if i<10 or i==31:
                    if i in ex:
                        args.wbits=3
                        args.groupsize=-1
            ...
            if args.tbits==3:
                if '7b' in args.model.lower(): 
                    if '0.25' in args.model.lower() and i<12 or i==31:
                        args.wbits=4
                        args.groupsize=-1
                    elif '0.5' in args.model.lower() and i<5 or i==31:
                        args.wbits=4
                        args.groupsize=-1
                    else:
                        args.wbits=3
        ...
        print(f'Quantizing layer {i+1}/{len(layers)} with {args.wbits} bits and groupsize of {args.groupsize}..')
        print('+------------------+--------------+------------+-----------+-------+')
        ...
        if args.true_sequential:
            if args.svd:
                sequential = [
                    ['self_attn.k_v_proj','self_attn.k_u_proj', 'self_attn.v_proj', 'self_attn.q_v_proj', 'self_attn.q_u_proj'],
                    ['self_attn.o_proj'],
        ...
        for names in sequential:
            ...
            gptq = {}
            for name in subset:
                if args.svd and name in ['self_attn.k_v_proj','self_attn.k_u_proj', 'self_attn.q_v_proj', 'self_attn.q_u_proj']:
                    args.wbits=3
                    args.groupsize=-1
            ...
            for name in subset:
                uv_scale=1
                if name.split('.')[-1] in ['k_v_proj','q_v_proj']:
                    uv_scale=10
                elif name.split('.')[-1] in ['k_u_proj','q_u_proj']:
                    uv_scale=0.1

                scale, zero, g_idx, error = gptq[name].fasterquant(percdamp=args.percdamp, groupsize=args.groupsize, actorder=args.act_order, name=name,uv_scale=uv_scale)
...
def llama_eval(model, testenc, dev):
    ...
    dtype = next(iter(model.parameters())).dtype
    inps = torch.zeros((nsamples, model.seqlen, model.language_model.config.hidden_size), dtype=dtype, device=dev)
    cache = {'i': 0, 'attention_mask': None}
    ...
    if model.language_model.model.norm is not None:
        model.language_model.model.norm = model.language_model.model.norm.to(dev)
    ...
    for i in range(nsamples):
        ...
        if model.language_model.model.norm is not None:
            hidden_states = model.language_model.model.norm(hidden_states)
...
def load_quant(model, checkpoint, wbits, groupsize=-1, fused_mlp=False, eval=True, warmup_autotune=True,seqlen=2048):
...
def llama_multigpu(model, gpus, gpu_dist):
    model.language_model.model.embed_tokens = model.language_model.model.embed_tokens.to(gpus[0])
    if hasattr(model.language_model.model, 'norm') and model.language_model.model.norm:
        model.language_model.model.norm = model.language_model.model.norm.to(gpus[0])
    import copy
...
if __name__ == '__main__':
    ...
    parser.add_argument('dataset', type=str, choices=['wikitext2', 'ptb', 'c4','videochatgpt','LLaVAVideo178K','LLaVA-OneVision-Data','LLaVA-Instruct-150K'], help='Where to extract calibration data from.')
    parser.add_argument('--seed', type=int, default=0, help='Seed for sampling the calibration data.')
    parser.add_argument('--nsamples', type=int, default=128, help='Number of calibration data samples.')
    parser.add_argument('--percdamp', type=float, default=.01, help='Percent of the average Hessian diagonal to use for dampening.')
    parser.add_argument('--nearest', action='store_true', help='Whether to run the RTN baseline.')
    parser.add_argument('--wbits', type=int, default=16, choices=[2, 3, 4, 8, 16], help='#bits to use for quantization; use 16 for evaluating base model.')
    parser.add_argument('--tbits', type=int, default=16, choices=[2, 3, 4, 8, 16], help='#bits to use for quantization; use 16 for evaluating base model.')
    ...
    parser.add_argument('--save_fake', type=str, default='', help='Save the fake quantized checkpoint under this name.')
    ...
    parser.add_argument('--svd', action='store_true', help='whether the attention is compressed')
    ...
    if args.load:
        model = load_quant(args.model, args.load, args.wbits, args.groupsize,seqlen=args.seqlen)
    else:
        model = get_llava(args.model,args,seqlen=args.seqlen)
        model.eval()
    ...
    if args.save_fake:
        from transformers import LlamaTokenizer, TextStreamer
        tokenizer = LlamaTokenizer.from_pretrained('llama-2-7b-hf', device_map="cpu", trust_remote_code=True)
        torch.save({
            'model': model,
            'tokenizer': tokenizer
        }, args.save_fake) 
    ...
    if args.eval:
        ...
        if args.new_eval:
            datasets = ['wikitext2', 'c4-new',  'ptb-new']
