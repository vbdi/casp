...
parser.add_argument('--base_model',
                    default='/home/ma-user/work/xinglu/LLMs/llama-2-7b-hf',
                    type=str)
parser.add_argument('--save_path', default='/home/ma-user/work/quip-sharp/hessians/llama-2-7b-hf', type=str)
...
def forward_layer(layer, position_ids, attention_mask, bs, device, in_q,
    ...
    if compressed_model:
        done_q_u = utils.register_H_hook(layer.self_attn.q_u_proj, device)
        done_q_v = utils.register_H_hook(layer.self_attn.q_v_proj, device)
        done_k_u = utils.register_H_hook(layer.self_attn.k_u_proj, device)
        done_k_v = utils.register_H_hook(layer.self_attn.k_v_proj, device)
...
def main(args):
    ...
    if args.compressed_model:
        ...
        model = LlavaNextVideoForConditionalGeneration.from_pretrained(args.base_model, 
    ...
    if os.path.isfile(f"{args.save_path}/dev_activations.pt"):
        ...
        dev_emb = model.language_model.model.embed_tokens(devset)
        after_layer = -1
    ...
    if hasattr(model.language_model.config, 'sliding_window'):
        attention_mask = _prepare_4d_causal_attention_mask(
            None, (args.batch_size, args.ctx_size),
            dev_emb[0:args.batch_size],
            0,
            sliding_window=model.language_model.config.sliding_window)
    else:
        attention_mask = _prepare_4d_causal_attention_mask(
            None, (args.batch_size, args.ctx_size),
    ...
    for transformer_layer_index in range(len(model.language_model.model.layers)):
        ...
        transformer_layer = model.language_model.model.layers[transformer_layer_index]
        ...
        model.language_model.model.layers[transformer_layer_index] = None
        utils.clean()

        if args.save_activations and (
                transformer_layer_index % args.act_save_rate == 0 or \
                transformer_layer_index == len(model.language_model.model.layers) - 1):
            if args.scratch_path is not None:
